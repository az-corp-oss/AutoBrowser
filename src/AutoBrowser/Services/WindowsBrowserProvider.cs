...
public class WindowsBrowserProvider : IBrowserProvider
{
    private IReadOnlyList<BrowserDefinition>? _cache;

    public IReadOnlyList<BrowserDefinition> GetInstalledBrowsers()
    {
        return _cache ??= ScanForBrowsers();
    }

    private static List<BrowserDefinition> ScanForBrowsers()
    {
    ...
    private static void ScanAppPathsSubKey(RegistryKey appPaths, List<BrowserDefinition> browsers)
    {
        foreach (var subKeyName in appPaths.GetSubKeyNames())
        {
            try
            {
                ...
                var exePath = BrowserDefinition.ParseExePath(path);
                ...
                var known = BrowserDefinition.ResolveDisplayName(name);
    ...
    private static void ScanUrlAssociationsRegistry(List<BrowserDefinition> browsers)
    {
        try
        {
            ...
            var path = BrowserDefinition.ParseExePath(cmd);
        ...
        try
        {
            ...
            foreach (var appName in registeredApps.GetValueNames())
            {
                try
                {
                    ...
                    var exePath = BrowserDefinition.ParseExePath(cmd);
                    if (string.IsNullOrEmpty(exePath) || !File.Exists(exePath)) continue;
